...
<form action="https://api.web3forms.com/submit" method="POST" on:submit={handleSubmit}>
    ...
    <div class="fields-container">
        <div>
            ...
            on:invalid={() => this.setCustomValidity('Vul hier uw voornaam in')}
        ...
        <div>
            ...
            on:input={saveToLocalStorage}
            on:invalid={(event) => event.target.setCustomValidity('Vul hier uw achternaam in')}
            on:input={(event) => event.target.setCustomValidity('')} />
        </div>
        ...
        <div>
            ...
            on:input={saveToLocalStorage}
            on:invalid={(event) => event.target.setCustomValidity('Vul hier uw e-mail in')}
            on:input={(event) => event.target.setCustomValidity('')} />
        </div>
        ...
        <textarea id="message" name="message" cols="30" rows="10" placeholder="Typ hier je bericht" required bind:value={message} 
            on:input={saveToLocalStorage} 
            on:invalid={(event) => event.target.setCustomValidity('Vul hier uw vraag of bericht in')}
            on:input={(event) => event.target.setCustomValidity('')} ></textarea>
    </div>
